...
parser.add_argument('-s','--host',help="Host for this server. ex: 0.0.0.0:8080",required=False,default="0.0.0.0:8080")
...
parser.add_argument("-r","--priority",help="Minimum trigger priority. Only trigger events equal to or above this will show up. 0 - (default) not classified 1 - information 2 - warning 3 - average 4 - high 5 - disaster.",default=0,type=int)
...
def makeRecord(zabbix,event):
    try:
        if event["object"] == '0':
            ...
            title = (description.replace("{HOST.NAME}",host)) if "{HOST.NAME}" in description else (host+": "+description)
...
class root:
    def GET(self):
        ...
        events = zabbix.event.get({"limit":100,"sortfield":"clock","sortorder":"DESC","extendoutput":True,"selectHosts":"extend","selectRelatedObject":"extend"})
